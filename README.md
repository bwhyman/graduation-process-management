...
- 任务，学生，提交开题答辩记录。学生查看开题答辩意见，复制评委教师意见，给出回答，提交文档。开题答辩记录文档，要作为毕业答辩考察依据。设置文件命名规则，可打包下载，主任，秘书？
...
- 状态。0未开始，1开始，2结束。status，smallint
- 任务类型，下注。type，smallint
- 完成角色，任务由那种角色完成。默认1。也有对教师下发任务。target，1学生，2教师。smallint
- 发布者ID，user_id, BIGINT(19)
- 模板文件名称。单独的模板文件目录，去找同名文件，可覆盖，文件名冲突要有提示。filename，varchar(45)
- 上传文件名称规则，下注。按固定规则命名。动态命名，创建任务时指定命名规则？指导教师姓名-学生姓名-学号-任务后缀(例如，开题答辩记录)，后缀手动输入。固定可选选项，只能选择顺序，约定选项名称，单独提供一个选项表？静态吧。一旦确定不可修改，否则，如果学生已经上传，文件已经由此命名，修改会很麻烦。可以直接删除任务，创建新任务. js的字符串数组[]，1指导教师姓名，2学生姓名，3学号，手动输的文件后缀名称。例如。学生姓名-学号-开题答辩记录，[2, 3, 开题答辩记录]
- 文件类型后缀，*.doc, *.docx等。直接存字符串。ext_name, varchar(8)

可选命名项，指导教师姓名，学生姓名，学号，毕设题目，自定义后缀(创建任务时输入)。后端直接以字符串数组保存，前端

js的字符串数组[]，1指导教师姓名，2学生姓名，3学号，4毕设题目，手动输的文件后缀名称。例如。学生姓名-学号-开题答辩记录，[2, 3, 开题答辩记录]
